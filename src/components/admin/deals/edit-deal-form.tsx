...
export function EditDealForm({ id, deal }: EditDealFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    active: deal.active,
    activeDate: deal.activeDate
      ? typeof deal.activeDate === "string"
        ? (deal.activeDate as string).split("T")[0]
        : (deal.activeDate as Date).toISOString().split("T")[0]
      : "",
    ...
    try {
      ...
      await updateRestaurantDeal(BigInt(id), {
        ...
        activeDate: formData.activeDate
          ? new Date(formData.activeDate + "T12:00:00").toISOString()
          : null,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="col-span-2">
        ...
        />
        <p className="mt-1 text-xs text-gray-500">
          Enter the date when the deal will be active
        </p>
      </div>
