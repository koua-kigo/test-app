  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 border border-gray-100">
      <div className="relative h-32 w-full mb-3">
        <Image
          src={prize.imageUrl || "https://via.placeholder.com/400x250?text=Prize"}
          alt={prize.name}
          fill
      ...
      <p className="text-gray-600 text-sm mb-2 line-clamp-2">{prize.description}</p>
...
async function RestaurantDetail({ id }: { id: string }) {
  ...
  const restaurantPrizes = await db.query.prizes.findMany({
    where: eq(prizes.restaurantId, restaurant.id),
  });
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        {restaurantPrizes.length > 0 ? (
          ...
          <p className="text-gray-600">No prizes available at this restaurant yet.</p>
        )}
      </div>
      
      {!isAuthenticated && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-8">
          <h3 className="text-lg font-semibold mb-2">Join our rewards program!</h3>
          <p className="text-gray-700 mb-4">
            Sign up to start collecting stamps and earning rewards at this restaurant.
          </p>
